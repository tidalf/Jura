...
class Device:
    ...
    async def read_maintenance_percents(self) -> dict:
        ...
        _LOGGER.info(f"Maintenance types ({len(self.maintenance_types)}): {self.maintenance_types}")
        _LOGGER.info(f"Maintenance data ({len(data)} bytes): {[b for b in data[:20]]}")
        ...
        _LOGGER.info(f"Maintenance results: {cleaning_percents}")
